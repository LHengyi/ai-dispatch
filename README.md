...
| Field | What to enter |
|-------|--------------|
| LLM provider | `gemini` (free), `openai` (paid), or `anthropic` (paid) |
...
| Secret | Value |
|--------|-------|
| `GEMINI_API_KEY` | Free key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey) — or use `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if you chose those providers |
...
To switch providers, update `provider` and the matching provider-specific model field in `config.yml` (`digest.gemini_model`, `digest.openai_model`, or `digest.anthropic_model`).
...
| 字段 | 填写内容 |
|------|----------|
| LLM provider | `gemini`（免费）、`openai`（付费）或 `anthropic`（付费） |
...
| Secret 名称 | 填写内容 |
|-------------|----------|
| `GEMINI_API_KEY` | 在 [aistudio.google.com/apikey](https://aistudio.google.com/apikey) 免费申请；若选 OpenAI / Anthropic，则改为 `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` |
...
切换 provider 只需改 `config.yml` 中的 `provider` 和对应模型字段（`digest.gemini_model`、`digest.openai_model` 或 `digest.anthropic_model`）。
