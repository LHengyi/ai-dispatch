...
To switch providers, update `provider` and the matching provider-specific model field in `config.yml` (`digest.gemini_model` or `digest.anthropic_model`).
...
切换 provider 只需改 `config.yml` 中的 `provider` 和对应模型字段（`digest.gemini_model` 或 `digest.anthropic_model`）。
