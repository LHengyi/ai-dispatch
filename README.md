...
**Your weekly AI intelligence briefing, delivered to your inbox.**

Automatically aggregates the latest in AI, Robotics, and Agents every week — analyzed by an LLM of your choice, delivered to your inbox. Runs entirely on GitHub Actions. No server. No subscription.
...
| 💡 This Week's Signal | The one judgment that matters most this week, in one sentence |
...
| Field | What to enter |
|-------|--------------|
| LLM provider | `gemini` (free), `deepseek` (paid), `openai` (paid), or `anthropic` (paid) |
...
| Secret | Value |
|--------|-------|
| `GEMINI_API_KEY` | Free key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey) — or use `DEEPSEEK_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if you chose those providers |
...
══════════════════════════════════════════════════════
  🎉  All checks passed! Your weekly digest is ready.
══════════════════════════════════════════════════════
```

Once all green, AI Dispatch runs automatically every week. The weekly digest is scheduled exactly once every Monday at `00:07` UTC, which is Monday `08:07` in Beijing time.
...
The default workflow is scheduled for Monday `02:07` UTC, which is Monday `10:07` in Beijing. The website audit time is now controlled directly by `.github/workflows/website_audit.yml`.
...
- The weekly email includes a structured broken-link list with URLs, captured anchor text when available, and suggested fixes.
...
- The website audit workflow is separate from the weekly news workflow, so the two jobs do not block each other.
- The default scheduled audit runs exactly once a week at Monday `02:07` UTC / Monday `10:07` Beijing time.
...
| Model | Cost | Notes |
|-------|------|-------|
| `gemini-2.0-flash` | Free | Default — 1,500 requests/day limit, easily enough for a weekly digest |
...
| Model | Cost | Notes |
|-------|------|-------|
| `deepseek-v4-flash` | See official DeepSeek pricing | Default DeepSeek provider model |
| `deepseek-v4-pro` | See official DeepSeek pricing | Higher-end DeepSeek option |

**Optional (OpenAI / ChatGPT API — paid):**

| Model | Cost | Notes |
|-------|------|-------|
| `gpt-5.5` | See official OpenAI pricing | Default OpenAI provider model |
| Other OpenAI model IDs | See official OpenAI pricing | You can override `digest.openai_model` in `config.yml` |

To switch providers, update `provider` and the matching provider-specific model field in `config.yml` (`digest.gemini_model`, `digest.deepseek_model`, `digest.openai_model`, or `digest.anthropic_model`).
...
├── website_audit.py        ← Website crawl + link audit + LLM email report
├── check_setup.py          ← Setup verification script
├── requirements.txt
├── sent_history.json       ← Auto-maintained dedup log (do not edit manually)
├── website_audit_history.json  ← Website audit send-state tracker
└── .github/workflows/
    ├── daily_news.yml      ← Weekly news cron job
    ├── website_audit.yml   ← Website audit cron job
...
**Q: Test email arrived but no weekly digest?**
Check Actions → AI Dispatch Weekly for errors. GitHub Actions cron can occasionally delay 15–30 minutes.
...
**Q: Why did the website audit miss some links?**
The current audit uses static HTTP crawling. If a page injects links with JavaScript after load, those links may not be discovered.
...
| 字段 | 填写内容 |
|------|----------|
| LLM provider | `gemini`（免费）、`deepseek`（付费）、`openai`（付费）或 `anthropic`（付费） |
...
| Secret 名称 | 填写内容 |
|-------------|----------|
| `GEMINI_API_KEY` | 在 [aistudio.google.com/apikey](https://aistudio.google.com/apikey) 免费申请；若选 DeepSeek / OpenAI / Anthropic，则改为 `DEEPSEEK_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` |
...
══════════════════════════════════════════════════════
  🎉  所有检查通过！查收测试邮件后即可等待每周简报。
══════════════════════════════════════════════════════
```

全部绿色后每周自动运行。新闻周报固定在每周一 `UTC 00:07` 发送，也就是北京时间每周一 `08:07`。
...
默认 workflow 固定在 `UTC 周一 02:07` 触发，也就是北京时间周一 `10:07`。网站巡检的发送时间现在直接由 `.github/workflows/website_audit.yml` 里的 cron 控制。
...
- 每周巡检邮件会附带结构化坏链清单：包含失效链接 URL、已捕获到的 anchor text，以及建议修复动作。
...
- 默认定时巡检现在每周只会在“北京时间周一上午 10:07”触发一次。
...
| 模型 | 费用 | 说明 |
|------|------|------|
| `gemini-2.0-flash` | 免费 | 默认，每天 1500 次请求限额，周报绰绰有余 |
...
| 模型 | 费用 | 说明 |
|------|------|------|
| `deepseek-v4-flash` | 参考 DeepSeek 官方定价 | 默认 DeepSeek provider 模型 |
| `deepseek-v4-pro` | 参考 DeepSeek 官方定价 | 更强的 DeepSeek 选项 |

**OpenAI / ChatGPT API（付费，可选）：**

| 模型 | 费用 | 说明 |
|------|------|------|
| `gpt-5.5` | 参考 OpenAI 官方定价 | OpenAI provider 默认模型 |
| 其他 OpenAI 模型 ID | 参考 OpenAI 官方定价 | 可在 `config.yml` 里覆盖 `digest.openai_model` |

切换 provider 只需改 `config.yml` 中的 `provider` 和对应模型字段（`digest.gemini_model`、`digest.deepseek_model`、`digest.openai_model` 或 `digest.anthropic_model`）。
...
├── setup.py                ← 交互式配置向导
├── fetch_news.py           ← 主程序
├── website_audit.py        ← 网站抓取、坏链检测、LLM 邮件报告
├── check_setup.py          ← 配置验证脚本
├── requirements.txt
├── sent_history.json       ← 已推送博客记录（自动维护，请勿手动编辑）
├── website_audit_history.json  ← 网站巡检发送状态记录
└── .github/workflows/
    ├── daily_news.yml      ← 每周新闻定时任务
    ├── website_audit.yml   ← 网站巡检定时任务
...
**Q: 测试邮件收到了但每周邮件没来？**
检查 Actions → AI Dispatch Weekly 里有没有报错。GitHub Actions 的 cron 有时会延迟 15–30 分钟。
...
**Q: 为什么网站巡检漏掉了一些链接？**
当前巡检基于静态 HTTP 抓取。如果页面是在加载后由 JavaScript 动态插入链接，这些链接可能不会被发现。
